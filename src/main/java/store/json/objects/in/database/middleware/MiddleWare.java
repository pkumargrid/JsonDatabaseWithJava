    ...
    public static void set(JsonElement keyArray, JsonElement value, DataHandler dataHandler) {
       ...
       if(keyArray.isJsonPrimitive()){
           ...
           if(value.isJsonPrimitive()){
               dataHandler.setData(key, value.getAsString());
           }
           else{
               Gson gson = new Gson();
               Map<String, Object> nestedObject = gson.fromJson(value, Map.class);
               dataHandler.setData(key, nestedObject);
           }
           return;
       }
       ...
       if(value.isJsonPrimitive()){
           dataHandler.setData(lastKey, value.getAsString());
           map.put(lastKey, value.getAsString());
       }
       else{
           Gson gson = new Gson();
           Map<String, Object> nestedObject = gson.fromJson(value, Map.class);
           dataHandler.setData(lastKey, nestedObject);
       }
    }
}
